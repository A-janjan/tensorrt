"""
!git clone https://github.com/kamata1729/QATM_pytorch.git
%cd QATM_pytorch
"""
...
class Featex():
    def __init__(self, model1, model2, use_cuda):
        ...
        self.input_shape_2 = (1, 3, 225, 384)
        ...
        """
        self.model1 = layer1to2_trt
        self.model2 = layer1to16_trt
        """
        # self.model= c opy.deepcopy(model.eval())
    ...
    def __call__(self, input, mode='big'):
        ...
        self.feature1 = self.model1(input).detach() #need to be changed
        self.feature2 = self.model2(input).detach() #need to be changed
...
class CreateModel():
    def __init__(self, alpha, model1, model2, use_cuda):
        self.alpha = alpha
        self.featex = Featex(model1, model2, use_cuda)
        self.I_feat = None
...
native_model = models.vgg19(pretrained=True).features
native_model2 = models.vgg19(pretrained=True).features


model = CreateModel(model1=native_model, model2=native_model2, alpha=25, use_cuda=True)
print("="*30,"THIS IS TRT MODEL", "="*30)

five_inference_times=0
for i in range(5):
	start = time.time()
	scores, w_array, h_array, thresh_list = run_multi_sample(model, dataset)
	boxes, indices = nms_multi(scores, w_array, h_array, thresh_list)
	end = time.time()
	five_inference_times += (end-start)
	
inference_time = five_inference_times*1000
inference_time /= 5
print("======> inference time: ",inference_time," milisec <==============")
